...
public final class CustomUtil {
     ...
     * @return
     */
    public static Map<Locale,String> getLocalizationMap(Map<String,Object> param,String parameter){
    	if(CustomUtil.strNull(param.get(parameter)).equals("")) {
    		return null;
    	}else{
     ...
     * @param param - RequestUtil을 통하여 request에서 변환된 Map
     * @param parameter - 다국어가 적용된 Parameter
     * @param defaultValue - 해당 값이 null 일 경우 defaultValue로 Setting
     * @return
     */
    public static Map<Locale,String> getLocalizationMap(Map<String,Object> param,String parameter,String defaultValue){
    	...
    	if(!param.containsKey(parameter)) {
        	...
        	Map<Locale, String> map = new HashMap<Locale, String>();
        	List<Locale> nullLoacleList = new ArrayList<Locale>();
        	String nullDataReplace = defaultValue;
     ...
     * @return
     */
    public static Map<Locale,String> getLocalizationNotSetLocaleMap(Map<String,Object> param,String parameter){
    	if(CustomUtil.strNull(param.get(parameter)).equals("")) {
    		return null;
    	}else{
    		Set<Locale> locales = LanguageUtil.getAvailableLocales();
     ...
     * @return
     */
    public static Map<Locale,String> getLocalizationNotSetLocaleMap(Map<String,Object> param,String parameter,String defaultValue){
    	if(!param.containsKey(parameter)) {
    		return null;
    	}else{
    		Set<Locale> locales = LanguageUtil.getAvailableLocales();
        	...
        	for (Locale locale : locales) {
    			...
    			map.put(locale, strNull(param.get(localeParameter),defaultValue));
        	}
        	return map;
    	}
    }
    ...
    /**
     * 다국어에 따른 기관명 가져오기
     * @param orgXmlString - ExpandoValue에서 꺼낸 data
     * @param locale
     * @return
     */
	public static String getLocalizedOrganizationName(long organizationId, long companyId, Locale locale) {
		String organizationName = "";
		try {
			if(organizationId != 0) {
				long organizationClassNameId = ClassNameLocalServiceUtil.getClassNameId(Organization.class);
				ExpandoTable expandoTable = ExpandoTableLocalServiceUtil.getTable(companyId, organizationClassNameId, "CUSTOM_FIELDS");
				ExpandoColumn expandoColumn = ExpandoColumnLocalServiceUtil.getColumn(expandoTable.getTableId(), "Name");
				String data = ExpandoValueLocalServiceUtil.getValue(expandoTable.getTableId(), expandoColumn.getColumnId(), organizationId).getData();
				
				Document document = SAXReaderUtil.read(data);
				List<Node> nodes = document.selectNodes("/root/Data[@language-id=\""+locale+"\"]");
				organizationName = nodes.get(0).getText();
			}
		} catch (DocumentException | PortalException e) {
			System.out.println("Get Organization Error.");
			e.printStackTrace();
		}
	...
	/**
	 * 왼쪽 자리수 채우기
	 * @param inputString - 원본데이터
	 * @param length      - 데이터사이즈
	 * @param strChar     - 대체 문자열
	 * @return
	 */
	public static String stringLpad(String inputString, int length, String strChar) {
	 ...
	 * @return (A3 : 국가 이름)
	 */
	public static LinkedHashMap<String, String> getCountryList(Locale locale) {
		Map<String,String> countryMap = CountryServiceUtil.getCountries()
				.stream()
				.collect(Collectors.toMap(Country::getA3, model->model.getName(locale)));
		
		return sortMapByKey(countryMap);
	}
